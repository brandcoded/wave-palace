...
> Last updated: 2026-06-27 (Discord guild-join add-on complete)
...
| **11** | **Host Onboarding & Ownership** | **✅ COMPLETE (v0.14.0)** | — | `Channel.owner_ids` + `auto_publish` (admin-only, stripped from public API) · `ChannelInviteToken` schema + `InviteRepository` (Seed + Mongo, `channel_invites`) · single-use 7-day SHA-256-hashed tokens · `InviteService` generate/accept/list · `POST/GET /api/admin/channels/{slug}/invites` · `POST /api/host/invite/accept` · `GET /api/admin/channels/{slug}/owners` · `require_channel_owner` dep (admin/MD bypass) · `get_channels_by_owner` repo method · admin Ownership panel (hosts, remove, invite link, auto_publish toggle) · `/host/join?token=` page with `SignInPanel` · 19 backend tests (251 total) · TS clean |
| **12** | **Logged-In Dashboard** | **✅ COMPLETE (v0.16.0)** | — | `listen_events` + `channel_saves` + `notifications` collections · `ListenHistoryService` + `RecommendationService` · `POST/GET /api/me/history` · `POST /api/me/history/merge` · `POST/DELETE/GET /api/me/saves` · `GET/PATCH /api/me/notifications` · `POST /api/me/notifications/mark-all-read` · `GET /api/me/recommendations` · `GET /api/me/follows` · `GET /api/me/channels` · `/home` page (greeting, resume, notifications, history, saves, recommendations, creator panel) · `UserMenuIsland` (avatar + unread badge + dropdown) · `ChannelCard` heart/save button (optimistic, auth-gate) · `ChannelPlayer` fires listen event on each track · `SignInPanel` merge after secret login · `get_optional_user` dep · 20 backend tests (304 total) · TS clean |
| **13 add-on** | **Discord guild-join on OAuth callback** | **✅ COMPLETE (v0.17.2)** | — | Auto-PUTs follower into WavePalace Discord server on `/callback` using `guilds.join` scope + Discord Add Guild Member API; fixes `50278` DM errors; graceful degradation (all failures logged, never 500); `DISCORD_GUILD_ID` env var; 9 backend tests (339 total) |
| **13 add-on** | **Public Metrics Display** | **✅ COMPLETE** | — | `follower_count`, `listener_count`, `worlds_count`, `trending` fields on `Channel` schema · `active_listener_count(slug)` using 15-min window of `_PLAY_CACHE` · `asyncio.gather` parallel follower counts on `GET /api/channels` and single-channel enrichment on `GET /api/channels/{slug}` · `channel_created_at` on `FollowPublicView` for "Early listener" badge · `metrics.ts` threshold-based display utilities (never show zero) · `ChannelCard` metric row (live dot + plays + followers) + Trending badge · `ChannelPlayer` overlay metrics line · `ChannelGrid` 60s visibility-gated polling · `/home` taste reflection ("Your sound: X · Y · Z") from listen history + channel tags · `/follows` "Early listener" private badge within 90 days of channel creation · 9 backend tests (330 total) · TS clean |
| **13** | **Notification System** | **✅ COMPLETE (v0.17.0)** | — | Per-follow prefs (`notify_new_tracks`, `notify_channel_live`, `notify_digest`) with Pydantic backfill · `ThrottleRepository` (Seed + Mongo, TTL index 7d) · `email_templates.py` (new_tracks, channel_live, weekly_digest) · `NotificationDeliveryService` (Resend email + Discord bot DM, throttle, inbox notif creation) · new-track detection hook in `PATCH /api/admin/channels/{slug}` (asyncio.create_task) · `POST /api/admin/channels/{slug}/notify` (manual trigger, ignore_throttle) · `POST /api/admin/notifications/digest` (cron endpoint) · `PATCH /api/follows/{id}` extended to accept notify_* fields · `/follows` page: expandable preferences panel + `?unsubscribe={follow_id}` deep link · `.env.example` created · 17 backend tests (321 total) · TS clean |
